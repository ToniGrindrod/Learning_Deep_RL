...
class actorNetwork(nn.Module):
    def __init__(self, input_size, hidden_dimensions1,hidden_dimensions2, out_features):
        super(actorNetwork, self).__init__()

        # Fully connected layers (MLP)
        self.fc1 = nn.Linear(input_size, hidden_dimensions1)
        self.fc2 = nn.Linear(hidden_dimensions1, hidden_dimensions2)    
        self.fc3 = nn.Linear(hidden_dimensions2, out_features)

    def forward(self, state, avail_action):
        x = F.relu(self.fc1(state))
        x = F.relu(self.fc2(x))  # First fully connected layer with ReLU activation
        x = self.fc3(x)
...
class PPO_agent:
    def __init__(self,
        ...
        self.actor = actorNetwork(
            self.INPUT_FEATURES, hidden_dimensions1,hidden_dimensions2, self.ACTOR_OUTPUT_FEATURES
        ).to(self.device)
        self.critic = criticNetwork(
            self.INPUT_FEATURES, hidden_dimensions1,hidden_dimensions2, self.CRITIC_OUTPUT_FEATURES
        ).to(self.device)
    ...
    def update_policy(self,
        ...
        for _ in range(self.PPO_steps):
            ...
            for batch_idx, (states,avail_actions,actions,actions_log_probability_old, advantages, returns) in enumerate(batch_dataset):
                ...
                #             return
                ##
                # Apply gradient clipping to the model's parameters
...
class SMAC_agents: #could rename to SMAC execution
    def __init__(self,
        ...
        self.device = device
        self.run = run
        self.n_steps = n_steps# number of steps per training update
        self.total_steps=total_steps #total number of steps over which training happens
        ...
        self.episode_rewards=[]#find the number of complete epsiodes by finding the length of this array
        self.current_num_steps=0
        self.num_agents = self.env.n_agents
        #self.env.reset()
        agents_actions= self.env.n_actions
        num_agents_actions= [agents_actions for t in range(self.num_agents)]
        #print(num_agents_actions)
        agents_obs=self.env.get_obs_size()
        num_agents_obs = [agents_obs for t in range(self.num_agents)]
        ...
        self.PPO_agents = [PPO_agent(
                 ...
                 num_agents_obs[j],
                 grad_norm) for j in range(self.num_agents)]
        self.discount_factor = discount_factor
    ...
    def calculate_gae(self, rewards, values, dones):
        ...
        Args:
            rewards (list or tensor): Rewards collected from the environment.
            values (tensor): Estimated state values from the critic.
            dones (list or tensor): Boolean flags for episode termination.

        Returns:
            advantages (tensor): GAE advantage estimates.
            returns (tensor): Discounted returns for updating the value function.
        """
        advantages = []
        gae = 0
        returns = []
        ...
        # Append a zero for the last value (bootstrapped)
        values = torch.cat((values, torch.tensor([0.0], device=self.device)))

        for t in reversed(range(len(rewards))):
            delta = rewards[t] + self.discount_factor * values[t + 1] * (1 - dones[t]) - values[t]
            gae = delta + self.discount_factor * self.lambda_gae * (1 - dones[t]) * gae
            advantages.insert(0, gae)
        #print(type(advantages),type(values))

        advantages = torch.tensor(advantages, dtype=torch.float32, device=values.device)
        returns = advantages + values[:-1]  # Compute returns for the value function update
    ...
    def init_training_all(self):
        ...
        rewards = []
        dones = [] # Initialize an empty list to store the done flags
        step_count=0
        return states, avail_actions, actions, actions_log_probability, values, rewards, dones, step_count
    
    def forward_pass_all(self, state, avail_action, done, total_reward):
        states, avail_actions, actions, actions_log_probability, values, rewards, dones, step_count = self.init_training_all()
        ...
        # #state = self.env.get_obs() this does the same thing as the above line
        # avail_action = self.env.get_avail_actions()
        # No reset here! We already got state, avail_action from train_with_test
        
        while step_count < self.n_steps:#used to be while True
            if done:#if episode ends before reach n_steps, start a new episode
                self.episode_rewards.append(total_reward)
                episode=len(self.episode_rewards)
                #print("episode length:", len(rewards))
                print("episode",episode,": ", total_reward)
                self.run["episode_reward"].log(total_reward)
                # Calculate the mean of recent rewards and losses for display
                # mean_train_rewards = np.mean(self.episode_rewards[-self.n_trials:])
        ...
        for i in range(self.num_agents):
            ...
            advantage, returns = self.calculate_gae(rewards, values[i], dones)
            returns_array.append(returns)
            advantages.append(advantage)
            
        # Return the updated state and avail_action to continue from the correct point
        return states, avail_actions, actions, actions_log_probability, advantages, returns_array, state, avail_action, done, total_reward
    
    def train_with_test(self):
        # Reset the environment only at the start of training
        done=False
        total_reward=0
        state, _ = self.env.reset()
        avail_action = self.env.get_avail_actions()
        while self.current_num_steps<self.total_steps:#notice where i update self.current_num_steps+=1. the while condition in this line might only get checked after current_num_steps has already been incremented.
            states, avail_actions, actions, actions_log_probability, advantages, returns, state, avail_action, done, total_reward = self.forward_pass_all(state, avail_action, done, total_reward)
            #now pass these to update policy states_batch.append(states)
            
            # Update the policy using the experience collected
            
            for i in range(self.num_agents):
                _, _ = self.PPO_agents[i].update_policy(
                    states[i],
                    avail_actions[i],
                    actions[i],
                    actions_log_probability[i],
                    advantages[i],
                    returns[i]) #used to return  policy_losses and value_losses which wehre then returned by train
        return
...
def mainSMAC():
    ...
    params = {
        ...
        "BATCH_SIZE":1536,# stable baselines was 64
        "n_steps": 2048,#from stable baselines
        "total_steps":2050000,#from the other example i think
        "grad_norm":0.5,
        "lambda_gae":0.95,
    }
    ...
    agents =SMAC_agents(env= env,
                 run = run,
                 device= device,
                 hidden_dimensions1 = params["HIDDEN_DIMENSIONS1"],
                 hidden_dimensions2 = params["HIDDEN_DIMENSIONS2"],
                 ...
                 batch_size=params["BATCH_SIZE"], n_steps=params["n_steps"], total_steps= params["total_steps"],
                 grad_norm=params["grad_norm"],
                 lambda_gae=params["lambda_gae"])


    agents.train_with_test()
    # Plotting the rewards and losses
    run.stop()
    #print(train_rewards)

    train_rewards = np.array(agents.episode_rewards)
