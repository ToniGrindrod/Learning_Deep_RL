...
class BackboneNetwork(nn.Module):
    def __init__(self, in_features, hidden_dimensions, out_features, dropout):
        super().__init__()
        #BackboneNetwork class inherits from nn.Module, which is the base class for all neural networks in PyTorch.
        #Using super().__init__() ensures that the __init__() method of nn.Module is called, setting up the necessary internal structure.

        self.layer1 = nn.Linear(in_features,hidden_dimensions)
        self.layer2 = nn.Linear(hidden_dimensions, hidden_dimensions)
        self.layer3 = nn.Linear(hidden_dimensions, out_features)
        #self.dropout = nn.Dropout(dropout)# creates a Dropout layer in PyTorch, used to reduce overfitting in neural networks.
        #Dropout  randomly sets a fraction of the input units (neurons) to zero during training (prevents the model from becoming overly reliant on specific neurons)
        #dropout parameter specifies the probability of dropping (zeroing out) each input unit.
    def forward(self, x):
        x = self.layer1(x)
        x = torch.relu(x)
        #x = self.dropout(x)
...
class PPO_agent:
    def __init__(self,
        ...
        self.INPUT_FEATURES = self.env.observation_space.shape[0]
        self.ACTOR_OUTPUT_FEATURES = self.env.action_space.n
    ...
    def forward_pass(self):#this is just the training function (might just want to rename it)
        ...
        return episode_reward, states, actions, actions_log_probability, advantages, returns, episode_len
    ...
    def update_policy(self,
        ...
        for _ in range(self.PPO_steps):
            for batch_idx, (states,actions,actions_log_probability_old, advantages, returns) in enumerate(batch_dataset):
                ...
                value_loss.backward()
                self.optimizer.step() # update the agent's parameters using the computed gradients (via the optimization algorithm)
    ...
    def train_with_test(self):
        ...
        for episode in range(1, self.max_episodes + 1):
            ...
            train_reward, states, actions, actions_log_probability, advantages, returns, episode_len = self.forward_pass()
...
def mainLunar():
    my_api="eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vYXBwLm5lcHR1bmUuYWkiLCJhcGlfdXJsIjoiaHR0cHM6Ly9hcHAubmVwdHVuZS5haSIsImFwaV9rZXkiOiJjZDg3ZjNlYi04MWI3LTQ1ODctOGIxNS1iNTY3ZjgzMGYzMzYifQ=="
    run = neptune.init_run(
    project="EnergyGridRL/Lunar-PPO",
    api_token=my_api
    )  # your credentials
    # Create the environment
    params = {
        ...
        "HIDDEN_DIMENSIONS": 64,
        "DROPOUT": 0.2,
        "LEARNING_RATE":0.001,#0.003 to 5e-6
        "BATCH_SIZE":32
    }
